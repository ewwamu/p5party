...
// Color palette
const palette = ['tomato', 'darkorange', 'gold', 'chartreuse', 'aqua'];
...
function preload() {
	...
	testImg = loadImage('https://picsum.photos/id/19/200/200')
}
...
function mousePressed() {
	...
	const p3 = guests.find((p) => p.role === 'gold');
	...
	if (p1 === me || p2 === me || p3 === me || p4 === me || p5 === me) {
		drawColor = me.role;
		// 5 boxes by 5 boxes
...
function drawUI() {
	...
	// Display timer
	textFont('Times New');
	text(shared_time.gameTimer, 10, 60);
}
		...
		for (let col = 0; col < 5; col++) {
			...
			if (shared.grid[col][row]) {
				...
				if (currentColor === 'tomato') {
					image(testImg, x + 1, y + 1, 80 - 2, 80 - 2,)
